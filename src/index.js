import { pages, handleBase } from './mappings.js';
...
export default {
  async fetch(request, env) {
    ...
    // 3. 페이지 렌더링 (render 함수 존재 여부 체크)
    if (targetPage && typeof targetPage.render === 'function') {
      ...
      const response = await fetch(`https://file.chkip.org/languages.json`);
      let lang = { ...originalTexts };
      
      const countryCode = (request.cf?.country || 'KR');

      if(response.ok) {
        const targetLangName = countryMap[countryCode];
        const languageJson = await response.json();
        const language = languageJson[targetLangName];
      ...
      const pageUrl = `${siteUrl}${url.pathname}`;

      const ip = request.headers.get('cf-pseudo-ipv4') || request.headers.get('cf-connecting-ip') || "8.8.8.8";
      const lat = request.cf?.latitude || 37.5665;
      const lng = request.cf?.longitude || 126.9780;
      const city = request.cf?.city || 'Unknown';
      const isp = request.cf?.asOrganization || 'ISP';

      const regionNames = new Intl.DisplayNames(['en'], { type: 'region' });
      const countryName = regionNames.of(countryCode || 'KR');
      const countryTimeZone = request.cf?.timezone || 'Asia/Seoul';

      const data = {
        pageTitle, pageDesc, siteUrl, pageUrl,
        ip, lat, lng, city, isp, 
        countryCode, countryName, countryTimeZone,
        lang
      };

      return new Response(targetPage.render(data), {
        headers: { "Content-Type": "text/html; charset=utf-8" }
      });
    }

    return new Response("<h1>404: 페이지를 찾을 수 없어!</h1>", {
      status: 404,
      headers: { "Content-Type": "text/html; charset=utf-8" }
    });
  }
