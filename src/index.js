...
function getLanguage(cf) {
  ...
  const lang = languages[targetLangName];
  return lang;
}
...
export default {
  async fetch(request, env) {
    ...
    if (targetPage && typeof targetPage.render === 'function') {
      ...
      const lang = getLanguage(request.cf);
      const data = {
        pageTitle, pageDesc, siteUrl, pageUrl,
        ip, lat, lng, city, isp, 
        countryCode, countryName, countryTimeZone, url
      };

      return new Response(targetPage.render(data,lang), {
